...
function FormClases({ formClase, setFormClase }: FormProp) {
  ...
  const eliminarHorario = (index: number) => {
    const horariosFiltrados = formClase.horarios.filter((_, i) => i !== index);
    setFormClase({
      ...formClase,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="border-t border-slate-200 pt-6">
          <button
            type="submit"
            disabled={cargando}
            className="w-full rounded-lg bg-indigo-600 px-5 py-3 font-medium text-white shadow-sm transition hover:bg-indigo-700 hover:shadow-md disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500 disabled:hover:bg-slate-300"
          >
            Crear clase
          </button>
        </div>
