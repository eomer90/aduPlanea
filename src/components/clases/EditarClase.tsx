...
function EditarClase({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await obtenerClaseSeleccionada();
    } catch (error) {
      console.log(error);
    } finally {
      setCargando(false);
          ...
          <div className="space-y-6">
            ...
            <section className="border-t border-slate-200 pt-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                  ...
                  className="w-full rounded-lg border border-indigo-200 px-3 py-2 text-sm font-medium text-indigo-600 transition hover:bg-indigo-50 sm:w-auto"
                >
                  + Agregar horario
                </button>
              </div>
              ...
              <div className="mt-4 space-y-3">
                {claseActualizada.horarios.map((horario, index) => (
                  <div
                    key={`${horario.dia}-${horario.inicio}-${horario.fin}-${index}`}
            ...
            <div className="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:justify-end">
              <button
                type="submit"
                disabled={cargando}
                className="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500 disabled:hover:bg-slate-300 sm:w-auto"
              >
                Guardar cambios
              </button>
              ...
              <button
                ...
                className="w-full rounded-lg border border-red-200 px-4 py-2.5 text-sm font-medium text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500 disabled:hover:bg-slate-300 sm:w-auto"
              >
                Eliminar clase
              </button>
            </div>
